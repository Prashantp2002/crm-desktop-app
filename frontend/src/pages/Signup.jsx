...
function Signup() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "username") setIsUsernameAuto(false);
  ...
  useEffect(() => {
    ...
    const cleanName = formData.fullname.trim().toLowerCase().replace(/\s+/g, "");

    let prefix = "";
    switch (formData.role) {
      case "admin":    prefix = "admin_";    break;
      case "employee": prefix = "employee_"; break;
      case "client":   prefix = "client_";   break;
      default:         prefix = "";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[0-9]{10}$/.test(formData.phone))     newErrors.phone    = "Enter valid 10 digit phone number";
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        setFormData({ fullname: "", phone: "", email: "", role: "", username: "", password: "" });
        navigate("/");
      } catch (error) {
